...
def build_prompt(data: dict) -> str:
    ...
    if investor.get("available"):
        ...
        investor_block = f"""\
5일 누적  외국인: {signed(foreign_net)}주 / 기관: {signed(institution_net)}주
{flow_signal}
...
▶ 기술적 지표 (당일 기준)
  ...
  [이동평균]
  MA5   : {fv(tech.get("ma5"))}원  (현재가 {ma_pos(tech.get("ma5"))})
  MA20  : {fv(tech.get("ma20"))}원  (현재가 {ma_pos(tech.get("ma20"))})
  MA60  : {fv(tech.get("ma60"))}원  (현재가 {ma_pos(tech.get("ma60"))})
  MA120 : {fv(tech.get("ma120"))}원  (현재가 {ma_pos(tech.get("ma120"))})
  크로스 신호 : {tech.get("ma_cross") or "없음"}

  [RSI-14]
  RSI     : {fv(tech.get("rsi"))}  → {tech.get("rsi_label", "")}
  RSI 추세: 최근 3일 {tech.get("rsi_trend") or "N/A"}

  [MACD (12, 26, 9)]
  MACD선    : {fv(tech.get("macd_line"), digits=4)}
  시그널선  : {fv(tech.get("signal_line"), digits=4)}
  히스토그램: {fv(tech.get("histogram"), digits=4)}  ({hist_dir})
  크로스    : {tech.get("macd_cross") or "없음"}
  상태      : {tech.get("macd_label", "")}

  [볼린저밴드 (20, 2σ)]
  상단 : {fv(tech.get("bb_upper"))}원
  중심 : {fv(tech.get("bb_middle"))}원
  하단 : {fv(tech.get("bb_lower"))}원
  %B   : {pct_b_str}  → {tech.get("bb_label", "")}
  밴드폭(변동성): {fv(tech.get("bb_width"), "%")}

  [스토캐스틱 (14, 3)]
  %K : {fv(tech.get("stoch_k"))}  %D : {fv(tech.get("stoch_d"))}
  상태: {stoch_label(tech.get("stoch_k"))}

  [거래량]
  20일 평균 거래량  : {int(tech.get("vol_ma20") or 0):,}주
  오늘/평균 비율    : {fv(tech.get("vol_ratio"))}배"""
    else:
    ...
    if fin.get("available"):
        ...
        def per_vs_industry(per, ind_per):
            if per is None or ind_per is None or ind_per == 0:
                return ""
            ratio = per / ind_per
        ...
        if mc:
            if mc >= 1_0000_0000_0000:  # 1조 이상
                mc_str = f"{mc / 1_0000_0000_0000:.1f}조원"
            elif mc >= 1_0000_0000:      # 1억 이상
                mc_str = f"{mc / 1_0000_0000:.0f}억원"
            else:
                mc_str = f"{mc:,}원"
        else:
            mc_str = "N/A"

        per_str = fv(fin.get("per"))
...
▶ 재무 지표 (최근 결산 기준)
  ...
  [밸류에이션]
  PER       : {per_str}  ({fin.get("per_label", "")})
  예상 PER  : {fv(fin.get("forward_per"))}
  PBR       : {fv(fin.get("pbr"))}  ({fin.get("pbr_label", "")})
  PSR       : {fv(fin.get("psr"))}
  EV/EBITDA : {fv(fin.get("ev_ebitda"))}
  시가총액  : {mc_str}

  [수익성]
  ROE          : {fv(fin.get("roe"), "%")}
  ROA          : {fv(fin.get("roa"), "%")}
  영업이익률   : {fv(fin.get("op_margin"), "%")}
  순이익률     : {fv(fin.get("net_margin"), "%")}
  매출총이익률 : {fv(fin.get("gross_margin"), "%")}

  [성장성 (전년 대비)]
  매출 성장률    : {fv(fin.get("revenue_growth"), "%")}{growth_label(fin.get("revenue_growth"))}
  순이익 성장률  : {fv(fin.get("earnings_growth"), "%")}{growth_label(fin.get("earnings_growth"))}
  분기 순이익 성장(QoQ) : {fv(fin.get("earnings_qoq"), "%")}{growth_label(fin.get("earnings_qoq"))}

  [재무 건전성]
  부채비율    : {fv(fin.get("debt_to_equity"))}%
  유동비율    : {fv(fin.get("current_ratio"))}
  당좌비율    : {fv(fin.get("quick_ratio"))}

  [배당]
  배당수익률  : {fv(fin.get("dividend_yield"), "%")}
  배당성향    : {fv(fin.get("payout_ratio"), "%")}"""
    else:
    ...
    oil_pct    = float(oil.get("change_pct", 0) or 0)
    ...
    # ══════════════════════════════════════════════════════════════
    prompt = f"""당신은 국내 증권사 15년 경력 수석 애널리스트입니다.
아래에 제공된 **실제 시장 데이터**만을 근거로 분석하세요.
막연한 일반론("글로벌 불안정성" 등)은 금지합니다. 반드시 제공된 수치를 인용하세요.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ 분석 대상: {company_name}  ({country} / {ticker})
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

▶ 현재 주가
  현재가  : {cur:,.0f}원
  전일 대비: {stock.get("change"):+,.0f}원  ({fmt_pct(stock.get("change_pct"))})
  52주 범위: 최저 {low:,.0f}원 ~ 최고 {high:,.0f}원
  52주 내 위치: {w52_pct:.1f}% → {w52_label}
  오늘 거래량: {vol:,}주  ({vol_label})

▶ 최근 5거래일 가격 흐름
  {price_trend}

▶ 시장 지표 (현재 기준)
  코스피      : {kospi.get("price")}  ({fmt_pct(kospi.get("change_pct"))})  [{direction_label(kospi.get("change_pct"))}]
  달러/원     : {usd_krw.get("price")}원  ({fmt_pct(usd_krw.get("change_pct"))})
  WTI 유가    : ${oil.get("price")}  ({fmt_pct(oil.get("change_pct"))})
  S&P500 선물 : {sp500_futures.get("price")}  ({fmt_pct(sp500_futures.get("change_pct"))})
  나스닥 선물 : {nasdaq_futures.get("price")}  ({fmt_pct(nasdaq_futures.get("change_pct"))})
  금          : ${gold.get("price")}  ({fmt_pct(gold.get("change_pct"))})
...
아래 10개 섹션을 **순서대로, 빠짐없이** 작성하세요.

## 1. 종합 시황 분석
- 코스피 {fmt_pct(kospi.get("change_pct"))}, 달러/원 {usd_krw.get("price")}원, 미국 선물 동향이
  {company_name}의 사업 특성(매출 구조, 환율 민감도 등)에 구체적으로 어떤 영향을 주는지 서술
- 유가·금 움직임의 간접 영향도 포함
- 오늘 거래량({vol_label})이 갖는 의미 해석

## 2. 뉴스·사회이슈 심층 분석
뉴스 감성 점수 {sentiment.get("score", 0):+.2f} (긍정 {sentiment.get("positive",0)}건 / 부정 {sentiment.get("negative",0)}건) 를 바탕으로:
- **핵심 이슈 3가지**: 주가에 가장 직접적 영향을 줄 뉴스를 골라 각각 "호재/악재/중립" 판정 및 영향 강도(강/중/약) 명시
- **카테고리별 해석**: 실적·규제·지정학 등 어떤 종류의 이슈가 지배적인지, 그 함의
- **시장 반영 여부**: 이미 주가에 선반영된 뉴스인지 vs 아직 미반영인지 판단 근거
- **사회·거시 이슈 연결**: 현재 금리 환경, 지정학 리스크, 산업 트렌드가 {company_name}에 미치는 구조적 영향

## 3. 재무 가치 분석
- PER {fin.get("per") if fin.get("available") else "N/A"} / PBR {fin.get("pbr") if fin.get("available") else "N/A"}: 현재 주가가 싼지 비싼지, 업종 평균 대비 위치
- ROE·영업이익률 수준과 경쟁사 대비 위치
- 매출/순이익 성장률 추세가 주가 방향에 주는 시사점
- 부채비율·유동비율로 본 재무 안정성 평가

## 4. 기술적 지표 분석
- RSI {tech.get("rsi") if tech.get("available") else "N/A"}: 현재 구간과 3일 추세 방향
- MACD 히스토그램 방향과 크로스 신호의 신뢰도
- 볼린저밴드 %B {f'{round(tech.get("bb_pct_b",0)*100,1)}%' if tech.get("available") and tech.get("bb_pct_b") is not None else "N/A"}: 밴드 위치와 변동성(밴드폭) 해석
- MA5/MA20/MA60 정배열/역배열 판단
- **지표 합의도**: RSI·MACD·BB·스토캐스틱이 같은 방향인지 충돌하는지 — 합의가 높을수록 신뢰도 높음

## 5. 상승 요인
수급·시황·뉴스·재무·기술적 지표에서 도출한 구체적 상승 근거를 3~5개 항목으로 작성.
각 항목마다 **어떤 수치·사실이 근거인지** 명시.

## 6. 하락 요인
수급·시황·뉴스·재무·기술적 지표에서 도출한 구체적 하락 근거를 3~5개 항목으로 작성.
각 항목마다 **어떤 수치·사실이 근거인지** 명시.

## 7. 외국인·기관 수급 심층 분석
- 5일 수급 흐름 추세(가속·둔화·전환)를 일별 데이터로 판단
- 외국인 보유 비중 변화 방향과 그 시사점
- 현재 수급 패턴과 유사한 역사적 패턴이 나타났을 때 일반적인 주가 경과

## 8. 내일 주가 예측
**반드시 아래 형식을 그대로 사용하세요 (자동 파싱용):**

**예측 방향**: 상승 / 하락 / 보합 중 택1
**예상 등락률**: XX% ~ XX%  (예: -1.5% ~ -0.3%)
**예측 근거**:
  1. (수치 기반 근거 1)
  2. (수치 기반 근거 2)
  3. (수치 기반 근거 3)
**핵심 가정**: 이 예측이 맞으려면 내일 어떤 조건이 유지되어야 하는가

## 9. 상승 타이밍 분석
단기(1~3일), 중기(1~2주), 장기(1개월 이상) 시점별로 구분하여 작성:

**단기 매수 타이밍**:
- 진입 조건: (어떤 지표/가격/이벤트가 충족되어야 하는가)
- 진입 목표가: {cur:,.0f}원 기준 XX원 부근
- 단기 목표가: XX원  (근거: )
- 손절 기준: XX원 이탈 시

**중기 시나리오**:
- 상승 시나리오: (어떤 조건이 갖춰지면, 어느 시점에 XX원 도달 가능)
- 하락 시나리오: (어떤 리스크가 현실화되면, 어느 수준까지 조정 가능)
- 분할매수 전략: (타이밍 분산 방법)

**핵심 트리거 이벤트**: 주가 방향을 결정할 향후 예정 이벤트나 발표 (실적·공시·이벤트 등)

## 10. 종합 신뢰도 및 리스크
**신뢰도**: 낮음 / 보통 / 높음 중 택1
**신뢰도 근거**: 지표 합의도와 데이터 품질 기반
**상방 리스크**: 예측보다 더 오를 수 있는 시나리오
**하방 리스크**: 예측보다 더 내릴 수 있는 시나리오
**내일 장 중 주목 변수**: 확인해야 할 핵심 지표 또는 이벤트

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
주의사항:
- 모든 수치는 위 데이터에서 직접 인용할 것
- "일반적으로", "보통", "흔히" 같은 막연한 표현 최소화
- 섹션 제목(## 1. 등)은 반드시 유지
- 한국어로 작성
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"""
