...
def build_prompt(data: dict) -> str:
    ...
    # ── 요일 감지 → 주말/월요일 변수 경고 ───────────────────────
        ...
        weekday = _dt.now().weekday()  # 0=월 ~ 6=일
        weekend_warning = ""

        if weekday == 5 or weekday == 6:  # 토·일
            weekend_warning = """
            ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
            ⚠️ 【주말 분석 — 매우 중요】
            지금은 주말입니다. 현재 데이터는 직전 금요일 종가 기준입니다.
            반드시 아래 주말 변수를 고려해서 분석하세요:

            1. 월요일 갭상승 빈번 패턴
            - 금요일 외국인 순매도 → 월요일 환매수 전환 패턴 매우 빈번
            - 주말 사이 글로벌 증시 반등 시 월요일 갭상승 가능성 높음
            - 특히 금요일에 과도한 하락이 있었다면 기술적 반등 가능성 고려

            2. 주말 사이 발생 가능한 변수
            - 미국 주요 경제지표 발표 (고용, CPI, FOMC 등)
            - 지정학 이슈 변화 (휴전, 협상, 긴장 고조)
            - 주요 기업 어닝 서프라이즈
            - 정책 발표 (금리, 관세, 규제)

            3. 예측 지침
            - 단순 기술적 지표만으로 하락 예측하지 말 것
            - 외국인/기관 수급이 과도한 매도였다면 반등 가능성 높게 볼 것
            - 신뢰도는 반드시 "낮음"으로 설정할 것
            - 예측 등락률 범위를 평소보다 2배 넓게 설정할 것
            ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"""
            ...
            ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
            ⚠️ 【금요일 분석 주의사항】
            오늘은 금요일입니다. 주말을 앞두고 있어 아래를 고려하세요:
            - 오후 장 막판 포지션 정리 매물로 인한 하락 과장 가능성
            - 월요일 갭상승/갭하락 변수 존재
            - 외국인 금요일 매도 → 월요일 환매수 패턴 주의
            - 신뢰도를 한 단계 낮게 설정하고 범위를 넓게 잡을 것
            ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"""
            ...
            ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
            💡 【월요일 분석 참고】
            오늘은 월요일입니다. 주말 변수가 이미 시가에 반영되었을 수 있습니다:
            - 갭상승/갭하락으로 시작한 경우 추가 방향성 주목
            - 주말 뉴스가 이미 선반영됐다면 되돌림 가능성 고려
            - 외국인 월요일 순매수 전환 여부가 핵심 변수
            ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"""
    ...
    period_config = {
        "tomorrow": {
            "label": "내일 (1일)",
            "focus": "단기 모멘텀·수급·뉴스 중심",
            "sections_override": None,  # 기존 섹션 유지
        },
        "week": {
            "label": "1주일 (5거래일)",
            "focus": "단기 추세·수급 흐름·주요 이벤트 중심",
            "sections_override": "week",
        },
        "month": {
            "label": "1개월 (약 20거래일)",
            "focus": "중기 추세·재무 가치·산업 흐름 중심",
            "sections_override": "month",
        },
    }
    pcfg = period_config.get(period, period_config["tomorrow"])
    ...
    if period == "tomorrow":
        ...
        prompt += (
            f"## 8. 1주일 주가 예측 (향후 5거래일)\n"
            f"**반드시 아래 형식을 그대로 사용하세요 (자동 파싱용):**\n\n"
            f"**예측 방향**: 상승 / 하락 / 보합 중 택1\n"
            f"**예상 등락률**: XX% ~ XX%  (예: -3% ~ +1%)\n"
            f"**주간 시나리오**:\n"
            f"  - 상승 시나리오: 어떤 조건이면 몇 % 상승 가능\n"
            f"  - 하락 시나리오: 어떤 리스크면 몇 % 하락 가능\n"
            f"  - 기준 시나리오: 가장 가능성 높은 방향과 근거\n"
            f"**주간 예상 가격 범위**: {cur:,.0f}원 기준 XX원 ~ XX원\n"
            f"**핵심 변수**: 이번 주 주가 방향을 결정할 이벤트/지표 3가지\n\n"
            f"## 9. 주간 매매 전략\n"
            f"**매수 구간**: XX원 ~ XX원  (근거: )\n"
            f"**1차 목표가**: XX원  (근거: )\n"
            f"**2차 목표가**: XX원  (근거: )\n"
            f"**손절 기준**: XX원 이탈 시\n"
            f"**분할매수 전략**: (1차 XX%, 2차 XX% 등 구체적으로)\n"
            f"**주중 주목 일정**: 실적발표·경제지표·이벤트 등\n\n"
            f"## 10. 종합 신뢰도 및 리스크\n"
            f"**신뢰도**: 낮음 / 보통 / 높음 중 택1\n"
            f"**신뢰도 근거**: 주간 예측의 불확실성 요인\n"
            f"**상방 리스크**: 예측보다 더 오를 수 있는 시나리오\n"
            f"**하방 리스크**: 예측보다 더 내릴 수 있는 시나리오\n"
            f"**이번 주 핵심 모니터링 포인트**: 매일 확인해야 할 지표\n"
        )

    elif period == "month":
        prompt += (
            f"## 8. 1개월 주가 예측 (향후 20거래일)\n"
            f"**반드시 아래 형식을 그대로 사용하세요 (자동 파싱용):**\n\n"
            f"**예측 방향**: 상승 / 하락 / 보합 중 택1\n"
            f"**예상 등락률**: XX% ~ XX%  (예: -5% ~ +10%)\n"
            f"**월간 시나리오**:\n"
            f"  - 강세 시나리오 (확률 XX%): 조건과 목표가\n"
            f"  - 기준 시나리오 (확률 XX%): 조건과 목표가\n"
            f"  - 약세 시나리오 (확률 XX%): 조건과 목표가\n"
            f"**예상 가격 범위**: {cur:,.0f}원 기준 XX원 ~ XX원\n"
            f"**중기 방향 전환 조건**: 어떤 신호가 나오면 방향이 바뀌는가\n\n"
            f"## 9. 중기 투자 전략\n"
            f"**분할매수 계획**:\n"
            f"  - 1차 매수: XX원 이하, 비중 XX%\n"
            f"  - 2차 매수: XX원 이하, 비중 XX%\n"
            f"  - 3차 매수: XX원 이하, 비중 XX%\n"
            f"**중기 목표가**: XX원  (근거: 재무가치/기술적 저항선)\n"
            f"**손절 기준**: XX원 이탈 시 (월봉 기준)\n"
            f"**보유 기간 전략**: 어떤 조건이 충족되면 매도할 것인가\n"
            f"**월간 핵심 이벤트**: 실적발표·배당·주요 경제지표 일정\n\n"
            f"## 10. 종합 신뢰도 및 리스크\n"
            f"**신뢰도**: 낮음 / 보통 / 높음 중 택1\n"
            f"**신뢰도 근거**: 중기 예측의 불확실성 요인\n"
            f"**상방 리스크**: 예측보다 더 오를 수 있는 시나리오\n"
            f"**하방 리스크**: 예측보다 더 내릴 수 있는 시나리오\n"
            f"**월간 핵심 모니터링 포인트**: 주간 단위로 확인해야 할 지표\n"
        )
