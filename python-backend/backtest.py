...
결과 파일:
  backtest_result.json  — 날짜별 예측 vs 실제 상세
  backtest_summary.txt  — 전체 정확도 요약
"""
...
def market_override(pred: dict, data: dict) -> tuple[dict, str | None]:
    ...
    반환: (보정된 pred, override 이유 or None)
    """
    ...
    # 하락 override 조건
    if sp500_pct <= -1.5:
        reasons.append(f"S&P500선물 {sp500_pct:+.2f}%")
    if kospi_pct <= -1.5:
        reasons.append(f"코스피 {kospi_pct:+.2f}%")
    if usd_pct >= 0.8:
        reasons.append(f"달러/원 {usd_pct:+.2f}% (외국인매도)")

    if len(reasons) >= 1 and (sp500_pct <= -1.5 or kospi_pct <= -1.5):
        new_pred = dict(pred)
        atr = (pred["pct_high"] - pred["pct_low"]) / 2
        new_pred["direction"] = "하락"
        new_pred["pct_low"]   = round(-atr * 2, 1)
        new_pred["pct_high"]  = round(-atr * 0.3, 1)
        return new_pred, "시장급락override: " + " / ".join(reasons)

    # 상승 override 조건
    up_reasons = []
    if sp500_pct >= 1.5:
        up_reasons.append(f"S&P500선물 {sp500_pct:+.2f}%")
    if kospi_pct >= 1.5:
        up_reasons.append(f"코스피 {kospi_pct:+.2f}%")

    if len(up_reasons) >= 1 and (sp500_pct >= 1.5 or kospi_pct >= 1.5):
        new_pred = dict(pred)
        atr = (pred["pct_high"] - pred["pct_low"]) / 2
...
def run_ai_prediction(data: dict) -> tuple[str, dict | None]:
    try:
        from ai_analyzer import build_prompt, _call_groq_stream
...
def run_self_review(result: dict) -> str:
    ...
    backtest 결과 dict 하나를 받아 분석 텍스트 반환.
    """
    pred  = result["pred"]
    ev    = result["eval"]
...
def print_summary(results: list[dict], reviews: dict | None = None) -> str:
    ...
    if total == 0:
        lines.append("  검증 가능한 결과 없음 (실제 가격 조회 실패)")
        summary = "\n".join(lines)
        print(summary)
        return summary
    ...
    dir_acc   = round(hits / total * 100, 1)
    range_acc = round(in_ranges / total * 100, 1)

    lines.append(f"  테스트 건수:     {total}건  (시장override: {overridden}건)")
    lines.append(f"  방향 정확도:     {hits}/{total} = {dir_acc}%")
    lines.append(f"  범위 정확도:     {in_ranges}/{total} = {range_acc}%")
    lines.append("")
    lines.append("  방향별 분석:")
    ...
    for r in verified:
        ...
        hit_mark   = "✅" if ev["hit"]      else "❌"
        range_mark = "✅" if ev["in_range"] else "❌"
        note = "🔀override" if r.get("override_reason") else ""
        lines.append(
            f"  {date:<12} {p['direction']:<5} {rng:<17} "
            f"{ev['actual_pct']:+.2f}%  {ev['actual_dir']:<5} {hit_mark}   {range_mark}   {note}"
        )
    ...
    if reviews:
        ...
        if wrong:
            ...
            for r in wrong:
                ...
                ev   = r["eval"]
                p    = r["pred"]
                review_text = reviews.get(date, "  (분석 없음)")
                lines.append(f"\n  [{date}] 예측:{p['direction']} → 실제:{ev['actual_dir']} ({ev['actual_pct']:+.2f}%)")
                if r.get("override_reason"):
                    lines.append(f"  Override 적용: {r['override_reason']}")
...
def main():
    ...
    parser.add_argument("--no-review", action="store_true", help="틀린 예측 자기분석 생략")
    ...
    for i, day in enumerate(test_days, 1):
        ...
        if not args.no_ai:
            print(f"    🤖 AI 예측 중...")
            ai_text, pred = run_ai_prediction(data)
            if pred:
                # 시장 급변 override 적용
        ...
        if actual and pred:
            ...
            hit_str   = "✅ 방향 적중" if ev["hit"]     else "❌ 방향 빗나감"
            range_str = "✅ 범위 적중" if ev["in_range"] else "❌ 범위 벗어남"
            print(f"    📊 실제: {actual['price']:,.0f} ({ev['actual_pct']:+.2f}%, {ev['actual_dir']}) → {hit_str} / {range_str}")
        elif actual:
        ...
        results.append({
            ...
            "collected_data":  data,           # 자기분석용 원본 데이터 보존
            "ai_response":     ai_text[:500] if ai_text else None,
        })
        print()

    # 결과 저장 (collected_data 제외하고 저장 — 용량 절약)
    save_results = []
    for r in results:
        sr = {k: v for k, v in r.items() if k != "collected_data"}
        save_results.append(sr)

    out_path = Path(args.out)
    out_path.write_text(json.dumps(save_results, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"💾 상세 결과 저장: {out_path}\n")

    # 틀린 예측 자기분석
    reviews = {}
    if not args.no_review and not args.no_ai:
        wrong_results = [r for r in results if r.get("eval") and not r["eval"]["hit"]]
        if wrong_results:
            print(f"🔍 틀린 예측 {len(wrong_results)}건 자기분석 중...\n")
...
def get_stock_data_as_of(ticker: str, as_of: datetime) -> dict:
    ...
    try:
        ...
        info = {}
        try:
            info = t.fast_info  # 52주 고저는 현재 기준이라 한계 있음
        except Exception:
            pass

        # 52주 고저: as_of 기준 1년 이전 ~ as_of 구간
        try:
            year_hist = t.history(
                start=(as_of - timedelta(days=365)).strftime("%Y-%m-%d"),
                end=end_dt.strftime("%Y-%m-%d"),
...
def print_summary(results: list[dict]) -> str:
    ...
    lines = []
    lines.append("=" * 60)
    lines.append(f"  StockOracle 백테스트 결과")
    lines.append("=" * 60)
    ...
    lines.append(f"  테스트 건수:     {total}건")
    lines.append(f"  방향 정확도:     {hits}/{total} = {dir_acc}%")
    lines.append(f"  범위 정확도:     {in_ranges}/{total} = {range_acc}%")
    lines.append("")
    lines.append("  방향별 분석:")
    if up_preds:
        up_hits = sum(1 for r in up_preds if r["eval"]["hit"])
        lines.append(f"    상승 예측: {len(up_preds)}건 → 적중 {up_hits}건 ({round(up_hits/len(up_preds)*100,1)}%)")
    if dn_preds:
        dn_hits = sum(1 for r in dn_preds if r["eval"]["hit"])
        lines.append(f"    하락 예측: {len(dn_preds)}건 → 적중 {dn_hits}건 ({round(dn_hits/len(dn_preds)*100,1)}%)")
    if flat_preds:
        ft_hits = sum(1 for r in flat_preds if r["eval"]["hit"])
        lines.append(f"    보합 예측: {len(flat_preds)}건 → 적중 {ft_hits}건 ({round(ft_hits/len(flat_preds)*100,1)}%)")

    lines.append("")
    lines.append("  날짜별 상세:")
    lines.append(f"  {'날짜':<12} {'예측':<5} {'예상범위':<16} {'실제등락':<8} {'실제방향':<5} {'방향':<4} {'범위'}")
    lines.append("  " + "-" * 62)
    for r in verified:
        p    = r["pred"]
        ev   = r["eval"]
        ...
        range_mark= "✅" if ev["in_range"] else "❌"
        lines.append(
            f"  {date:<12} {p['direction']:<5} {rng:<16} "
            f"{ev['actual_pct']:+.2f}%  {ev['actual_dir']:<5} {hit_mark}  {range_mark}"
        )
...
def main():
    parser = argparse.ArgumentParser(description="StockOracle 백테스트")
    parser.add_argument("--ticker",   required=True,  help="티커 (예: 005930.KS, AAPL)")
    parser.add_argument("--name",     required=True,  help="회사명")
    parser.add_argument("--country",  default="한국", help="한국 / 미국 / 일본")
    parser.add_argument("--start",    required=True,  help="시작일 YYYY-MM-DD")
    parser.add_argument("--end",      required=True,  help="종료일 YYYY-MM-DD")
    parser.add_argument("--period",   default="tomorrow", help="tomorrow / week / month")
    parser.add_argument("--interval", type=int, default=1, help="테스트 간격 (영업일)")
    parser.add_argument("--out",      default="backtest_result.json")
    parser.add_argument("--no-ai",    action="store_true", help="AI 호출 없이 데이터 수집만")
    args = parser.parse_args()
    ...
    for i, day in enumerate(test_days, 1):
        ...
        data = collect_historical_data(
            args.ticker, args.name, args.country, day, args.period
        )
        ...
        if not args.no_ai:
            ...
            ai_text, pred = run_ai_prediction(data)
            if pred:
                print(f"    📌 예측: {pred['direction']} ({pred['pct_low']:+.1f}% ~ {pred['pct_high']:+.1f}%)")
            else:
                print(f"    ⚠️  예측 파싱 실패 (응답에 형식 없음)")
        ...
        results.append({
            ...
            "actual":      actual,
            "eval":        ev,
            "ai_response": ai_text[:500] if ai_text else None,  # 저장 용량 절약
        })
        print()
    ...
    # 추가
    existing_data["tests"].append(new_test)
    ...
    print(f"💾 Test {test_id} 저장 완료 (파일: {out_path})\n")
